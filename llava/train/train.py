...
class ModelArguments: 
    ...
    # -------------------------------------------------------------------
    #vit_aggregate_layers: Optional[str] = field(default=None) # 除了base feature外，要选择的层级列表
...
def preprocess_multimodal(
    ...
    if not is_multimodal:
        return sources
    # 遍历 sources，即每个对话或文本片段 
    for source in sources:
        # 遍历 source 中的每个句子
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']: 
                # 先将 DEFAULT_IMAGE_TOKEN 从句子中移除并进行清理
...
def preprocess_v1(
    ...
    sep = conv.sep + conv.roles[1] + ": " # 人类指令和机器回复之间的字符
    for conversation, target in zip(conversations, targets):
        # 计算有效的token数量
        total_len = int(target.ne(tokenizer.pad_token_id).sum())
        # 将每轮对话中的指令部分进行掩码
...
class LazySupervisedDataset(Dataset): 
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]: 
            ...
            # 对指令微调样本需要获取本文指令的embed，而预训练样本则直接将文本embed设为none
            if self.data_args.version != 'plain':
                ...
                sentence_embed_tensor = torch.load(self.list_data_dict[i]['sentence_embed'])
...
def train(attn_implementation=None):
    ...
    # print(type(model_args.vit_base_layer))
    # print(model_args.vit_base_layer)
    # print(type(model_args.vit_aggregate_groups))
    # print(model_args.vit_aggregate_groups)
